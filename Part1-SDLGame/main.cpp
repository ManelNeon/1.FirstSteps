...
bool loadMedia()
{
	...
	/*
	//different sprites from a spritesheet
	...
	*/
	...
	/*
	if (!gModulatedTexture.loadFromFile("images/spiderman.png", gRenderer)) {
		printf("Failed to load spiderman image!\n");
		success = false;
	}*/
		...
		else
		{
			...
			while (!quit)
			{
				...
				while (SDL_PollEvent(&e) != 0)
				{
					...
					/*
					//code to input and change the color of the image
					else if (e.type == SDL_KEYDOWN) {
						switch (e.key.keysym.sym) {
						//Increase red
							case SDLK_q:
				...
				/*
				//testing this code lmfao, checking how fast the update cycle actually is, safe to say, its like an epilepsy attack
				bool isTrue = false;
				...
				/*
				//this code changes the color of the image every frame we do it too
				SDL_SetRenderDrawColor(gRenderer, 0xFF, 0xFF, 0xFF, 0xFF);
				SDL_RenderClear(gRenderer);

				//if we wanted to optimize this, we would only set the color when we pressed the button GAME PROGRAMMING
